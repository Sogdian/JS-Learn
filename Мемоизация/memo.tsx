...
//useMemo
  // Хук useMemo позволяющий кешировать результаты вычислений между ререндерами пока не изменилось состояние в списке его зависимостей
  //React обязательно вызовет её при первом рендере, а при последующих будет возвращать готовое значение
  export default function NumberList() {
    //Здесь мы используем хук useMemo и передаем ему функцию для вычисления.
    //Она будет запускаться только тогда, когда изменяются значения из массива — в нашем случае numbers и tab
...
//useCallback
  // Хук useCallback это хук в React, который позволяет кешировать функции между повторными рендерингами.
